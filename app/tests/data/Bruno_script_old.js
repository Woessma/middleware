...
// ======================================================
// Bruno Visualisierung des generierten IPS Bundles
// ======================================================
function ipsVisEscapeHtml(str) {
  return String(str == null ? "" : str)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function ipsVisGetBundle() {
  const raw = bru.getVar("ipsBundle");
  if (!raw) return null;
  return typeof raw === "string" ? JSON.parse(raw) : raw;
}

function ipsVisFindFirst(bundle, resourceType) {
...
function ipsVisFindByReference(bundle, reference) {
...
function ipsVisPatientName(patient) {
  if (!patient || !patient.name || !patient.name.length) {
...
function ipsVisResourceTypeCounts(bundle) {
  const counts = {};
...
function ipsVisRenderXhtml(div) {
  if (!div) {
    return '<p class="empty">Keine Narrative vorhanden</p>';
  }

  // FHIR Narrative ist XHTML und kann direkt gerendert werden.
  return div;
}

function ipsVisRenderStats(bundle, composition) {
  const counts = ipsVisResourceTypeCounts(bundle);
    ...
    .map(function(rt) {
      return `
        <div class="stat-card">
          ...
          <div class="stat-label">${ipsVisEscapeHtml(rt)}</div>
        </div>
      `;
    })
    .join("");
  ...
  return `
    <div class="stats-grid">
      <div class="stat-card primary">
        <div class="stat-value">${bundle && bundle.entry ? bundle.entry.length : 0}</div>
...
function ipsVisRenderCompositionSections(bundle, composition) {
  ...
  return composition.section
    .map(function(sec, index) {
      ...
      const entryLinks = entries
        .map(function(ref) {
          const target = ipsVisFindByReference(bundle, ref.reference);
          const label = ref.reference || "Reference";
          const type = target ? target.resourceType : "unresolved";

          return `
            <li>
              <code>${ipsVisEscapeHtml(label)}</code>
              <span class="muted">(${ipsVisEscapeHtml(type)})</span>
            </li>
          `;
        })
        .join("");
        ...
        <section class="section-card">
          <div class="section-header">
            <div>
              ...
              <h3>${ipsVisEscapeHtml(sec.title || "Ohne Titel")}</h3>
            </div>

            <div class="section-code">
              ${ipsVisEscapeHtml(sectionCode)}
            </div>
          </div>

          <div class="section-narrative">
            ${ipsVisRenderXhtml(sec.text && sec.text.div)}
          </div>
...
function ipsVisRenderResourceOverview(bundle) {
        ...
        ${bundle.entry
          .map(function(e, idx) {
            ...
            return `
              <tr>
                <td>${idx + 1}</td>
                <td><span class="pill">${ipsVisEscapeHtml(r.resourceType || "")}</span></td>
                <td><code>${ipsVisEscapeHtml(r.id || "")}</code></td>
                <td><code>${ipsVisEscapeHtml(e.fullUrl || "")}</code></td>
              </tr>
            `;
          })
          .join("")}
...
function ipsVisStyles() {
  return `
    <style>
      ...
      .section-narrative table {
        border-collapse: collapse;
        width: 100%;
        margin-top: 8px;
      }

      .section-narrative th,
      .section-narrative td {
        border: 1px solid var(--border);
        padding: 7px;
        text-align: left;
        vertical-align: top;
...
function ipsVisBuildHtml() {
  const bundle = ipsVisGetBundle();
  ...
  if (!bundle) {
    return `
      <html lang="de">
        ...
        <body>
          <h1>CH-IPS Visualisierung</h1>
          <div class="top-box">
            <p><b>Keine Variable <code>ipsBundle</code> gefunden.</b></p>
            <p class="muted">
              Bitte zuerst den IPS Builder im gleichen Post-Response Script ausführen.
            </p>
          </div>
        </body>
      </html>
  ...
  const composition = ipsVisFindFirst(bundle, "Composition");
  const patient = ipsVisFindFirst(bundle, "Patient");

  return `
    <html lang="de">
      <head>
        <meta charset="UTF-8" />
        <title>CH-IPS Bundle Visualisierung</title>
        ${ipsVisStyles()}
      </head>

      <body>
        <h1>CH-IPS Bundle Visualisierung</h1>

        <div class="top-box">
          <p><b>Bundle ID:</b> <code>${ipsVisEscapeHtml(bundle.id || "")}</code></p>
          <p><b>Timestamp:</b> ${ipsVisEscapeHtml(bundle.timestamp || "")}</p>
          <p><b>Patient:</b> ${ipsVisEscapeHtml(ipsVisPatientName(patient))}</p>
          <p class="muted">
            Quelle: Bruno Variable <code>ipsBundle</code>, erzeugt im Post-Response Script.
          </p>
        </div>

        <h2>Übersicht</h2>
        ${ipsVisRenderStats(bundle, composition)}

        <h2>IPS Sections</h2>
        ${ipsVisRenderCompositionSections(bundle, composition)}

        <h2>Ressourcenübersicht</h2>
        ${ipsVisRenderResourceOverview(bundle)}

        <h2>Raw JSON</h2>
        <pre class="raw-json">${ipsVisEscapeHtml(JSON.stringify(bundle, null, 2))}</pre>
      </body>
    </html>
  `;
}
